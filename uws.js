  ...
  message: async (socket, message, isBinary) => {
    ...
    switch (json.action) {
      case "connection": {
        ...
        app.publish(
          roomId,
          formatMessage({ action: "gameUpdate", data: { game } })
        );
        ...
        if (game.playersConnected === game.numberPlayer) {
          ...
          app.publish(
            roomId,
            formatMessage({ action: "gameWillStart", data: { game } })
          );
          ...
          setTimeout(() => {
            Object.values(game.initPlayerList).forEach((p, index) => {
              ...
              app.publish(
                player.id,
                formatMessage({ action: "gameInit", data: { player, game } })
              );
            });
            game.startGame();
            ...
            app.publish(
              roomId,
              formatMessage({ action: "gameStart", data: { game } })
            );
            app.publish(
              roomId,
              formatMessage({ action: "update", data: { ...game, deck: true } })
            );
          }, 4000);
        }
        socket.break;
        ...
        if (player.deck.cardPlayed != null && !game.stop) {
          ...
          if (playersLost.length > 0) {
            ...
            if (playersWin.length > 0) {
              ...
              const message = `player${playersType.join(
                "/"
              )} +${cardsPerLooser} card noob`;
              game.history.push(
